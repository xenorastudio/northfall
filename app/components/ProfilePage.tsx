          ...
          {profileData?.linkedProviders && profileData.linkedProviders.length > 0 && (
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              {profileData.linkedProviders.map((provider) => {
                ...
                const meta = linkedMeta[provider.id];
                if (!meta) return null;
                return (
                  <span key={provider.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium border border-white/[0.06] text-nf-muted">
          ...
          <div className="py-4">
            {profileData?.linkedProviders && profileData.linkedProviders.length > 0 ? (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {profileData.linkedProviders.map((provider) => {
                    const linkedMeta: Record<string, { color: string; bg: string; label: string; path: string }> = {
                      "google.com": {
