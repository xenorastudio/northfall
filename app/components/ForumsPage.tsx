...
const AI_PROMPT = `أنت مساعد NorthFall — مساعد ذكي احترافي لمنصة مجتمعية.

مهمتك:
تقديم إجابات دقيقة، مرتبة، ومفيدة للمستخدمين، مع فهم عميق للسؤال قبل الرد.
...
هدفك النهائي:
تقديم أفضل إجابة ممكنة، بشكل احترافي، واضح، ومفيد فعلاً للمستخدم.`;
          ...
          <div className="px-5 -mt-6 relative z-10 pb-4">
            <div className="flex items-end gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0 pb-0.5">
                <div className="flex items-center gap-2">
                  <p className="text-[15px] font-bold text-nf-text truncate">{profile.name}</p>
...
- هدفك: مساعدة المستخدمين في كل شيء
...
- ممنوع HTML نهائياً ممنوع ممنوع — لا تكتب: <big> <span> <div> <small> <h1> <h2> <b> <i> <em> <strong> <mark> <sub> <sup> أو أي tag HTML أبداً أبداً أبداً
...
- كود سطري: \`code\`
- بلوك كود: \`\`\`language\\ncode\\n\`\`\` — اكتب الكود كاملاً بدون قطع
- اقتباس: > نص
- مائل: *نص* — للكلمات المهمة داخل فقرة
- روابط: [النص](الرابط)

مثال على رد صحيح (اتبع هذا دائماً):
## تصميم المستويات المتقدمة
مقدمة عن الموضوع...
...
مثال على رد خاطئ (لا تفعل هذا أبداً):
<big>تصميم المستويات</big>     ← خطأ! لا تستخدم HTML
<span style="font-size:24px">عنوان</span>  ← خطأ! لا تستخدم HTML
**تصميم المستويات**     ← خطأ! لا تستخدم bold كعنوان
تصميم المستويات         ← خطأ! لا تكتب عنوان بدون ##
...
- مجالك الحصري: تطوير الألعاب، المنصة، البرمجة، التصميم، الشيدرات، الصوت في الألعاب
- إذا سُئلت عن موضوع خارج نطاقك (سياسة، طب، أنمي، رياضة، طبخ، سفر، دين، فلسفة) اعتذر فوراً: "أنا متخصص فقط في تطوير الألعاب ومنصة NorthFall. لا أستطيع المساعدة في هذا الموضوع."

أنت مساعد احترافي متقدم هدفه تحسين جودة المحتوى وتجربة المستخدم داخل المنتدى.`;
...
7. اكتب فقرة واحدة مدمجة وجذابة`;
    ...
    const WRITING_MODES: Record<string, { match: RegExp; systemPrompt: string }> = {
      ...
      "translate": { match: /ترجم لي|ترجم النص/, systemPrompt: "ترجم النص للعربية الفصحى بأسلوب طبيعي وسلس. أجب بالترجمة فقط." },
    ...
    const tick = () => {
      ...
      setAiTypingProgress(prev => {
        ...
        for (const m of msgs) {
          ...
          const step = Math.max(1, Math.floor(m.content.length / 80));
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="relative group shrink-0" onMouseEnter={() => fetchAuthorCache(activeThread.authorUid)}>
                          <div className="flex flex-col items-center gap-1.5 w-[52px]" onClick={() => openProfile(activeThread.authorUid, activeThread.authorName, activeThread.authorPhoto)}>
                            ...
                            <span className="text-[11px] font-bold text-nf-text hover:text-nf-accent cursor-pointer text-center">{activeThread.authorName}</span>
                          ...
                          <div className="absolute top-0 right-full mr-2 w-[520px] bg-nf-card rounded-xl shadow-2xl border border-nf-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                            ...
                            <div className="px-5 pt-1 pb-4">
                              <div className="flex items-center justify-between mb-1.5">
                                <div>
                                  <p className="text-[17px] font-bold text-nf-text">{activeThread.authorName}</p>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="relative group shrink-0 mb-2 inline-block" onMouseEnter={() => fetchAuthorCache(reply.authorUid)}>
                              <span className="flex items-center gap-2 cursor-pointer" onClick={() => openProfile(reply.authorUid, reply.authorName, reply.authorPhoto)}>
                                ...
                                <span className="text-[13px] font-medium text-nf-accent hover:text-nf-accent/80 transition-colors">{reply.authorName}</span>
                              ...
                              <div className="absolute top-full mt-1 right-0 w-[480px] bg-nf-card rounded-xl shadow-2xl border border-nf-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                                ...
                                <div className="px-4 pt-1 pb-3">
                                  <div className="flex items-center justify-between mb-1.5">
                                    <div>
                                      <p className="text-[16px] font-bold text-nf-text">{reply.authorName}</p>
                                      <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        {(() => {
                                          ...
                                          if (aiTypingProgress[msg.id] > 0 && aiTypingProgress[msg.id] < msg.content.length) {
                                            return (
                                              <>
                                                <div className="relative">
                                                  <div className="blur-[3px] opacity-20 absolute inset-0 pointer-events-none select-none">
                                            ...
                                            return (
                                              <div style={{ animation: aiTypingProgress[msg.id] === 0 && !msg.isTyping ? 'none' : 'blurReveal 0.8s ease-out' }}>
                                                <ReactMarkdown components={markdownComponents}>{msg.content}</ReactMarkdown>
                                              </div>
                                            );
                                          }
    ...
    <a href={href || "#"} onClick={e => { if (!e.ctrlKey && !e.metaKey && !e.shiftKey) { e.preventDefault(); onOpen(); } }} className={cn("block bg-nf-card rounded-xl mb-2.5 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md border border-nf-border/20 hover:border-nf-accent/20", isVisited ? "opacity-80 hover:opacity-100" : "")}>
      <div className="flex p-4 gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 text-[11px] text-nf-dim font-medium">
              <span onClick={e => { e.stopPropagation(); openProfile(thread.authorUid, thread.authorName, thread.authorPhoto); }} className="text-nf-text font-bold hover:text-nf-accent cursor-pointer">{thread.authorName}</span>
